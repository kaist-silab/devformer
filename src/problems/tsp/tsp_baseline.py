...
def solve_gurobi(directory, name, loc, disable_cache=False, timeout=None, gap=None):
    ...
    try:
        problem_filename = os.path.join(
            ...
            "{}.gurobi{}{}.pkl".format(
                name,
                "" if timeout is None else "t{}".format(timeout),
        ...
        if os.path.isfile(problem_filename) and not disable_cache:
            ...
            cost, tour = solve_euclidian_tsp_gurobi(
                loc, threads=1, timeout=timeout, gap=gap
            )
...
def calc_batch_pdist(dataset):
    ...
    return (
        torch.matmul(diff[:, :, :, None, :], diff[:, :, :, :, None])
        .squeeze(-1)
        .squeeze(-1)
        .sqrt()
    )
...
def nearest_neighbour(dataset, start="first"):
    ...
    current = start
    dist_to_startnode = torch.gather(
        dist, 2, current.view(-1, 1, 1).expand(batch_size, graph_size, 1)
    ).squeeze(2)
    tour = [current]
    ...
    for i in range(graph_size - 1):
        ...
        dist.scatter_(
            2, current.view(-1, 1, 1).expand(batch_size, graph_size, 1), np.inf
        )
        nn_dist = torch.gather(
            dist, 1, current.view(-1, 1, 1).expand(batch_size, 1, graph_size)
        ).squeeze(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_cuda", action="store_true", help="Disable CUDA (only for Tsiligirides)"
    )
    parser.add_argument("--disable_cache", action="store_true", help="Disable caching")
    parser.add_argument(
        "--max_calc_batch_size", type=int, default=1000, help="Size for subbatches"
    )
    parser.add_argument(
        ...
        if method == "nn":
            ...
            if method == "concorde":
                ...
                executable = os.path.abspath(
                    os.path.join(
                        "problems", "tsp", "concorde", "concorde", "TSP", "concorde"
                    )
                )

                def run_func(args):
                    return solve_concorde_log(
                        executable, *args, disable_cache=opts.disable_cache
                    )
                ...
                def run_func(args):
                    return solve_lkh_log(
                        executable, *args, runs=runs, disable_cache=opts.disable_cache
                    )
