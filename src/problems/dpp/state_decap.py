...
class StateDecap:
    ...
    def __init__(
        ...
        # If this state contains multiple copies (i.e. beam search) for the same instance, then for memory efficiency
        # the loc and dist tensors are not kept multiple times, so we need to use the ids to index the correct rows.
        self.ids = ids  # Keeps track of original fixed data index of rows

        # State
        self.first_a = first_a
        self.prev_a = prev_a
        self.visited_ = visited_  # Keeps track of nodes that have been visited
        self.lengths = lengths
        self.cur_coord = cur_coord
        self.i = i  # Keeps track of step
    ...
    def _replace(self, **kwargs):
        # Hacky way to replace attributes
        # if a kwarg is different from the current attribute, replace it
        # otherwise keep the current attribute
        return StateDecap(
            loc=kwargs.get("loc", self.loc),
            dist=kwargs.get("dist", self.dist),
            ids=kwargs.get("ids", self.ids),
            first_a=kwargs.get("first_a", self.first_a),
