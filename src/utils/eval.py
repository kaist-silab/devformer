...
from src.utils import load_model, move_to
from src.utils.data_utils import save_dataset
from src.utils.functions import parse_softmax_temperature
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
            ...
            results = list(
                itertools.chain.from_iterable(
                    pool.map(
                        eval_dataset_mp,
                        [
                            (dataset_path, width, softmax_temp, opts, i, num_processes)
    ...
    np.savetxt(
        "seed_sweep/ours_%d_temp%d_%d.txt"
        % (opts.seed, opts.softmax_temperature, opts.width[0]),
        costs,
        fmt="%f",
        delimiter=",",
    )
    ...
    if opts.o is None:
        results_dir = os.path.join(
            opts.results_dir, model.problem.NAME, dataset_basename
        )
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        if sequences is None:
            ...
            print("best_sequences:", sequences)
        duration = time.time() - start
        for seq, cost in zip(sequences, costs):
            if model.problem.NAME in ("tsp", "decap"):
